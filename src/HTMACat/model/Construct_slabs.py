...
def Construct_slabs(Path_Info,Model):
    """
    Build a simple surface and write the generated structures in VASP format.
        ...
        for j,natom_dop in enumerate(Natom_dop):
            if natom_dop == '0':
               ...
               for m,slab in enumerate(slabs):
                   write_vasp("%s_%s_%s.vasp" %(mname,mfacet,m), slab, direct=True, sort=[''], vasp5=True)
               ...
               if struct == ele_dop:
                  ...
                  for m,slab in enumerate(slabs):
                     write_vasp("%s_%s_%s_%s_%s.vasp" %(mname,ele_dop,mfacet,natom_dop,m), slab, direct=True, sort=[''], vasp5=True)
